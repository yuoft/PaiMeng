...
public class EntityGen {
    public static void onEntitySpawn(final BiomeLoadingEvent event) {
        addEntityToSpecificBiomes(event, EntityTypeRegister.BOAR.get(), 60, 2, 4,
                Biomes.JUNGLE, Biomes.BAMBOO_JUNGLE, Biomes.MODIFIED_JUNGLE);
        addEntityToAllBiomesExceptThese(event, EntityTypeRegister.BOAR.get(), 50, 4, 8, Biome.Category.FOREST);
    }

    //给定群系 category
    private static void addEntityToAllBiomesExceptThese(BiomeLoadingEvent event, EntityType<BoarEntity> type,
                                                        int weight, int minCount, int maxCount, Biome.Category categoryIn) {
        Biome.Category category = event.getCategory();
        if (category == categoryIn){
