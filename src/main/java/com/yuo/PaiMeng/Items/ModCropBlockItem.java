...
public class ModCropBlockItem extends BlockNamedItem {
    ...
    public ActionResultType onItemUse(ItemUseContext context) {
        ...
        switch (useBlockEnum) {
            case COMMON:
                if (blockGrow.isIn(BlockRegistry.commonFarmland.get()))
                    return resultType(context);
                else return funb(world, player);
            case FERTILE:
                if (blockGrow.isIn(BlockRegistry.fertileFarmland.get()))
                    return resultType(context);
                else return funb(world, player);
            case AQUATIC:
                if (blockGrow.isIn(BlockRegistry.aquaticFarmland.get()))
                    return resultType(context);
                else return funb(world, player);
            default:
