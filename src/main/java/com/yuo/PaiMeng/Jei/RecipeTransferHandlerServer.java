...
public class RecipeTransferHandlerServer {

    private RecipeTransferHandlerServer() {
    }

    /**
     * Called server-side to actually put the items in place.
     */
    public static void setItems(PlayerEntity player, Map<Integer, CookingTransferHandler.MatchingItem> slotIdMap, List<Integer> craftingSlots, List<Integer> inventorySlots, boolean maxTransfer, boolean requireCompleteSets) {
        Container container = player.openContainer;

        // grab items from slots
        Map<Integer, ItemStack> slotMap = new HashMap<>(slotIdMap.size());
        //修改处
        for (Map.Entry<Integer, CookingTransferHandler.MatchingItem> entry : slotIdMap.entrySet()) {
            Slot slot = container.getSlot(entry.getValue().getSlotIndex());
            final ItemStack slotStack = slot.getStack();
            if (slotStack.isEmpty()) {
                return;
            }
            ItemStack stack = slotStack.copy();
            stack.setCount(entry.getValue().getSlotCount());
        ...
        while (true) { // for each set
            ...
            for (Map.Entry<Integer, ItemStack> entry : required.entrySet()) { // for each item in set
                ...
                if (!itemFound || resultItemStackLimitReached) {
                    ...
                    // Reduce the size of the found slot. 修改处
